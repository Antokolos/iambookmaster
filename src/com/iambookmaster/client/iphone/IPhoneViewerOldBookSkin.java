...
public abstract class IPhoneViewerOldBookSkin implements EntryPoint{
	...
	//			
				...
				public void scrollHorizontal(int delta) {
					...
					if (Math.abs(delta)>clientWidth/4) {
						IPhoneViewerOldBookSkin.this.scrollHorizontal(delta>0);
					}
	//				if (listener != null) {
...
//			scrollPanel.setScrollDelay(4);
	...
	protected void scrollHorizontal(final boolean right) {
		stopScrollAnimation();
		...
		if (right) {
			animate = scrollPanel.canScrollRight();
		} else if (scrollPanel.canScrollLeft()) {
			//can scroll
			animate = true;
		...
		if (animate) {
			flipAnimation = new FlipAnimation(scrollPanel) {
				@Override
				public int ready() {
					if (right) {
	...
	public class FlipAnimation extends Timer{
		...
		public FlipAnimation(IPhoneScrollPanel panel) {
			node = panel.getElement().cloneNode(true);
			oldPos = panel.getHorizontalScrollPosition();
			DOM.setElementPropertyInt((Element)node, "scrollLeft",oldPos);
			layout.getElement().insertBefore(node, scrollPanel.getElement());
			scrollPanel.getElement().getStyle().setOpacity(0);
			scrollPanel.getElement().getStyle().setBackgroundColor("#F2E6D4");
//			Scheduler.get().scheduleDeferred(new Command(){
//				public void execute() {
					x = layout.getWidgetLeft(scrollPanel);
					newPos = ready();
					pos = x+newPos-oldPos;
					y = layout.getWidgetTop(scrollPanel);
					step = (oldPos-newPos)/4;
//				}
//			});
		}
		...
		public void cancel() {
			super.cancel();
			if (running==false && node != null) {
				//end of animation
				scrollPanel.getElement().getStyle().clearBackgroundColor();
				layout.setWidgetPosition(scrollPanel, x, y);
				layout.getElement().removeChild(node);
				scrollPanel.getElement().getStyle().setOpacity(100);
				node = null;
			}
		}
		...
		public void run() {
			pos = pos + step;
			layout.setWidgetPosition(scrollPanel, pos, y);
			scrollPanel.getElement().getStyle().setOpacity(100);
			if (step > 0) {
				if (pos>= x) {
					cancel();
...
//		viewCanvas.cancelAnimation();
	...
	public IPhoneViewerOldBookSkin() {
		super();
	}
...
//		}
		...
		public void clearWithAnimation(final boolean l2r) {
			stopScrollAnimation();
			flipAnimation = new FlipAnimation(scrollPanel) {
				...
				public int ready() {
					int correction;
					if (l2r) {
						correction = - scrollPanelWidth;
					} else {
						correction = scrollPanelWidth;
					}
					layout.setWidgetPosition(scrollPanel, layout.getWidgetLeft(scrollPanel)+correction, layout.getWidgetTop(scrollPanel));
					return scrollPanel.getHorizontalScrollPosition() + correction;
				}
...
//			}
		...
		public void setListener(IPhoneViewListener listener) {
			IPhoneViewerOldBookSkin.this.listener = listener;
		}

		void performDone() {
			if (flipAnimation != null) {
				flipAnimation.perform();
				performDoneEnd();
			} else {
				performDoneEnd();
			}
...
//			DeferredCommand.addCommand(checkScrolling);
		...
		public int getClientWidth() {
			return IPhoneViewerOldBookSkin.this.getClientWidth();
		}
