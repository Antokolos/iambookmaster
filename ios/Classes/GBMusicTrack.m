...
- (id)initWithPath:(NSString *)path
{
	...
	if (noErr != AudioFileOpenURL((CFURLRef)[NSURL fileURLWithPath:path], 0x01, kAudioFileCAFType, &audioFile))
	{
		...
		NSLog(@"GBMusicTrack Error - initWithPath: could not open audio file.");
		return nil;
	}
	...
	if (dataFormat.mBytesPerPacket == 0 || dataFormat.mFramesPerPacket == 0)
	{
		...
		if (maxPacketSize > gBufferSizeBytes)
		{
			...
			NSLog(@"GBMusicTrack Warning - initWithPath: had to limit packet size requested for file");
...
- (void)postTrackFinishedPlayingNotification:(id)object
{
	...
	// the track
	//is done without the notification observer(s) having to worry about thread safety and autorelease pools
	...
	//send notification
	if (resumeAfter != nil) {
		[resumeAfter stringByEvaluatingJavaScriptFromString:@"window.resumeMusic();"];
	}
}
